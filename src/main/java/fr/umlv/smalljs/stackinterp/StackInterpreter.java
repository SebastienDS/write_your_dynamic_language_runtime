		...
		for (;;) {
			switch (instrs[pc++]) {
				case Instructions.CONST -> {
					...
					var constValue = instrs[pc++];
					// push(...)
					push(stack, sp++, constValue);
				}
				case Instructions.LOOKUP -> {
					...
					var lookup = globalEnv.lookup(name);
					push(stack, sp++, encodeAnyValue(lookup, dict));
				...
				case Instructions.FUNCALL -> {
					...
					// decode qualifier
					var qualifier = load(stack, baseArg, QUALIFIER_BASE_ARG_OFFSET);
					...
					if (maybeCode == UNDEFINED) { // native call !
						// decode receiver
						var receiver = decodeAnyValue(load(stack, baseArg, RECEIVER_BASE_ARG_OFFSET), dict, heap);

						// decode arguments
						var args = new Object[argumentCount];
						for (var i = 0; i < argumentCount; i++) {
							args[i] = decodeAnyValue(load(stack, baseArg, i), dict, heap);
						}
